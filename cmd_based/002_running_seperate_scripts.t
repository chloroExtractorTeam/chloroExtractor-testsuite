...
foreach my $testset (@test_list)
{
    ...
    if (exists $testset->{todo})
    {
      TODO: {
	  local $TODO = $testset->{todo};
	  ...
	  {
	      like($stdout, $testset->{exp_stdout}, sprintf('%s with argument %s return with expected output', $testset->{script}, join(", ", @{$testset->{arg}})));
	  }

	  if (exists $testset->{exp_stderr})
	  {
	      like($stderr, $testset->{exp_stderr}, sprintf('%s with argument %s return with expected stderr', $testset->{script}, join(", ", @{$testset->{arg}})));
	  }
	}
    } else {
	  is(Test::Script::Run::last_script_exit_code(), $testset->{exp_ret}, sprintf('%s with argument %s return with exit code %d', $testset->{script}, join(", ", @{$testset->{arg}}), $testset->{exp_ret}));
